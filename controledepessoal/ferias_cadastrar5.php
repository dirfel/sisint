<?php
//para excluir registro de férias é usado esse arquivo
...
$sql = 'DELETE FROM ferias WHERE id_usr = '.$_GET['id_usr'].' AND id = '.$_GET['registry'];
$stmt = $p1->prepare($sql);
$res = $stmt->execute();
header('location: ferias_cadastrar1.php?id_usr='.$_GET['id_usr'].'&token2='.base64_encode('Informação excluída!'));
?>