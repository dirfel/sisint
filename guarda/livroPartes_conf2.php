...
if ($_SESSION['nivel_guarda'] == 'Administrador') {
    //prossegue pois o administrador tem acesso irrestrito aos livros de partes já encerrados
} else if (count($consulta_reg) > 0 and $consulta_reg[0]['idusuario'] <> $idusuario) { // linha não testada
  $msgerro = base64_encode('Já possui Livro de Partes cadastrado nessa data por outro usuário!');
...
$temp1 = $consulta_reg[0]['temp1'] ?? '';
$temp2 = $consulta_reg[0]['temp2'] ?? '';
$temp3 = $consulta_reg[0]['temp3'] ?? '';
$temp5 = $consulta_reg[0]['temp5'] ?? '';
$temp6 = $consulta_reg[0]['temp6'] ?? '';
$temp7 = $consulta_reg[0]['temp7'] ?? '';
$umid1 = $consulta_reg[0]['umid1'] ?? '';
$umid2 = $consulta_reg[0]['umid2'] ?? '';
$umid3 = $consulta_reg[0]['umid3'] ?? '';
$umid5 = $consulta_reg[0]['umid5'] ?? '';
$umid6 = $consulta_reg[0]['umid6'] ?? '';
$umid7 = $consulta_reg[0]['umid7'] ?? '';
...
if ($idLivroPartes <> "") {
    ...
    // WHERE (data_inicio <= :data AND data_termino > :data AND idlivro == :idlivro)");   -->era assim, não estava exibindo o punido quando editava o livro
