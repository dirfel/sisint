...
if (!isset($_POST['action'])) {
    ...
    $select_relatorio = $pdo1->prepare("SELECT * FROM $rel WHERE id = :id");
    ...
    $select_usuarios = $pdo2->prepare("SELECT usuarios.nomeguerra, idpgrad FROM usuarios WHERE id = :idmembro");
    ...
    $select_usuarios2 = $pdo2->prepare("SELECT usuarios.nomeguerra, idpgrad FROM usuarios WHERE id = :idusuario");
    $select_usuarios2->bindParam(":idusuario", $idusuario, PDO::PARAM_INT);
    $select_usuarios2->execute();
    while ($reg3 = $select_usuarios2->fetch(PDO::FETCH_ASSOC)) {
        $nome_usuarios2 = $reg3['nomeguerra'];
        $pg_usuarios2 = getPGrad($reg3['idpgrad']);
    }

    $sistema = base64_encode('GUARDA');
    $obs = $situacao_excluir . " ID" . $id_rel . ": " . $pg_usuarios . " " . $nome_usuarios . ": " . $situacao . ". Lançado pelo " . $pg_usuarios2 . " " . $nome_usuarios2 . ".";
