...
$idveiculo =  base64_decode(filter_input(INPUT_POST, "tkveiculo", FILTER_SANITIZE_SPECIAL_CHARS));
$cracha = filter_input(INPUT_POST, "cracha", FILTER_SANITIZE_STRING);
$destino = filter_input(INPUT_POST, "destino", FILTER_SANITIZE_STRING);
$cracha = mb_strtoupper($cracha, 'UTF-8');
...
while ($reg = $select_visitante->fetch(PDO::FETCH_ASSOC)) {
  $situacao_visitante = $reg["situacao"];
...
while ($reg2 = $select_veiculo->fetch(PDO::FETCH_ASSOC)) {
  $situacao_veiculo = $reg2["situacao"];
...
$stmtez = $pdo1->prepare("INSERT INTO rel_visitantes(idvisitante, data, hora, idusuario, situacao, idveiculo, destino) "
  . "VALUES (:visitante, :data, :hora, :idusuario, :situacao, :idveiculo, :destino)");
...
if (
  ...
  $idveiculo == '' or $idveiculo == ' ' or $cracha == '' or $cracha == ' ' or
...
} else if ($idveiculo > 0 and $situacao_veiculo == 1) {
  $msgerro = base64_encode('Erro na tentativa de registrar o acesso, veículo já saiu ou entrou no aquartelamento!');
  header('Location: visitantes1.php?token=' . $msgerro);
  exit();
} else if ($situacao_visitante == $situacao2) {
  $msgerro = base64_encode('Erro na tentativa de registrar o acesso, visitante já saiu ou entrou no aquartelamento!');
  header('Location: visitantes1.php?token=' . $msgerro);
  exit();
